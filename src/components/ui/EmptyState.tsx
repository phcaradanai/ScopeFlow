...
export default function EmptyState({ 
  ...
  return (
    <div className="h-full flex items-center justify-center bg-gradient-to-b from-[#121214] to-[#09090b]">
      <div className="max-w-2xl w-full mx-auto px-6 text-center flex flex-col items-center">
        <div className={`w-16 h-16 rounded-2xl border flex items-center justify-center mb-6 ${iconBgClass}`}>
          <Icon className={`w-8 h-8 ${iconColorClass}`} />
        </div>
        <h2 className="text-2xl font-bold text-text mb-3 text-balance leading-relaxed">{title}</h2>
        <p className="text-base text-text-dim mb-8 max-w-lg">{description}</p>
