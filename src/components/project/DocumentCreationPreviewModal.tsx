...
const DOCUMENT_TYPE_LABELS: Record<string, { label: string, icon: any }> = {
  ...
  quotation: { label: 'ใบเสนอราคา (Quotation)', icon: Receipt },
  invoice: { label: 'ใบแจ้งหนี้ (Invoice)', icon: FileText },
  cr: { label: 'คำขอเปลี่ยนแปลง (CR)', icon: GitPullRequest },
  dcr: { label: 'คำขอเปลี่ยนแปลงการพัฒนา (DCR)', icon: Code },
  sup: { label: 'แจ้งปัญหา (Support Request)', icon: LifeBuoy },
  ma: { label: 'แจ้งซ่อมบำรุง (MA Request)', icon: Wrench },
  acceptance: { label: 'รายการตรวจรับ (Acceptance Checklist)', icon: CheckSquare },
  brief: { label: 'Brief Intake', icon: FileText },
};
      ...
      <div className="modal-container max-w-lg">
        <div className="modal-header">
          <div className="modal-header-content">
            ...
            <p className="modal-subtitle">
              ระบบแนะนำให้สร้างเอกสารนี้เพื่อเดินหน้าโครงการต่อ
            </p>
          </div>
        ...
        <div className="modal-body p-6 space-y-4">
          <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 flex flex-col gap-3">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wide text-text-muted mb-1">Target Project</p>
              <p className="text-sm font-bold text-text">{projectName}</p>
            </div>
            
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wide text-text-muted mb-1">Document Type</p>
            ...
            <div>
              <p className="text-[10px] font-bold uppercase tracking-wide text-text-muted mb-1">Lifecycle Stage / Priority</p>
              <p className="text-xs text-text">{lifecycleStage}</p>
            </div>
          </div>

          <div className="rounded-xl border border-border bg-surface-2 p-4">
            <p className="text-[10px] font-bold uppercase tracking-wide text-text-muted mb-1">Why is this recommended?</p>
            <p className="text-sm font-bold text-text mb-2">{reason}</p>
            <p className="text-xs text-text-muted leading-relaxed">
              <span className="font-bold text-primary-light">Detail:</span> {recommendationWhy}
            </p>
          </div>
        </div>
