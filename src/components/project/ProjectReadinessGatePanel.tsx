    ...
    <section className="mt-5 rounded-3xl border border-border bg-surface/80 p-5 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0 flex-1 break-words">
          <div className="flex flex-wrap items-center gap-2 mb-2">
            <span className="badge bg-primary/15 text-primary-light border border-primary/25 text-[10px] break-words">{t('quality.readinessGateTitle')}</span>
          ...
          <h2 className="text-lg font-black text-text flex items-center gap-2 break-words">
            <CheckCircle2 className="w-5 h-5 text-primary shrink-0" /> {gate.headline}
          </h2>
          <p className="mt-1 text-sm text-text-muted leading-relaxed break-words">{gate.summary}</p>
          <p className="mt-1 text-xs text-text-dim leading-relaxed break-words">{t('quality.readinessGateDescription')}</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button type="button" onClick={onPrimaryAction} className="btn btn-primary text-xs min-h-[48px] whitespace-nowrap">
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-5 gap-2">
        {stageOrder.map(({ key, labelKey, icon: Icon }) => {
          ...
          return (
            <div key={key} className={`rounded-2xl border p-3 min-w-0 break-words flex flex-col h-full ${active ? 'border-primary/30 bg-primary/10 text-primary-light' : reached ? 'border-success/20 bg-success/10 text-success' : 'border-border bg-surface-2 text-text-muted'}`}>
              <Icon className="w-4 h-4 mb-2 shrink-0" />
              <p className="text-[11px] font-bold leading-snug break-words flex-1">{t(labelKey as any)}</p>
            </div>
          );
        })}
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_320px] gap-3">
        <div className="rounded-2xl border border-border bg-surface-2/70 p-4 min-w-0 break-words flex flex-col">
          <h3 className="text-sm font-bold text-text flex items-center gap-2 break-words">
            <AlertTriangle className="w-4 h-4 text-warning shrink-0" /> {t('quality.blockersTitle')}
          </h3>
          <div className="mt-3 space-y-2 flex-1">
            {gate.blockers.length === 0 && <p className="text-xs text-text-muted leading-relaxed break-words">{t('quality.noBlockers')}</p>}
            {gate.blockers.slice(0, 6).map((blocker, index) => (
              ...
              >
                <p className="text-xs font-bold flex items-center gap-2 break-words">
                  {blocker.kind === 'open_change_request' ? <GitPullRequest className="w-3.5 h-3.5 shrink-0" /> : <AlertTriangle className="w-3.5 h-3.5 shrink-0" />}
                  {blocker.label}
                </p>
                <p className="mt-1 text-xs leading-relaxed opacity-90 break-words">{blocker.reason}</p>
              </button>
            ))}
          </div>
        </div>

        <div className="rounded-2xl border border-primary/20 bg-primary/10 p-4 min-w-0 break-words flex flex-col">
          <p className="text-[11px] font-bold uppercase tracking-wide text-primary-light mb-2 break-words">{t('quality.nextAction')}</p>
          <h3 className="text-base font-black text-text break-words">{gate.primaryAction.label}</h3>
          <p className="mt-2 text-xs text-text-muted leading-relaxed break-words flex-1">{gate.primaryAction.description}</p>
          <button type="button" onClick={onPrimaryAction} className="btn btn-primary w-full justify-center text-xs mt-4 whitespace-nowrap">
            {gate.primaryAction.label}
          </button>
        </div>
      </div>
