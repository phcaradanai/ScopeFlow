...
import QuotationDraftPanel from './QuotationDraftPanel';
...
export default function BriefIntakeModal({ clientId, projectId, projectPath, onClose }: BriefIntakeModalProps) {
  ...
  const handleImproveBrief = async () => {
    if (!formData.raw_request.trim() || !workspacePath) return;
    try {
      setIsGeneratingAi(true);
      setError('');
      const { generateJsonWithTrace } = await import('../lib/ai/providers/aiProviderRouter');
      const prompt = `You are a professional business analyst. Rewrite the following customer request into a clear, professional, well-structured format. Keep it in Thai. Do not invent requirements that are not stated. Output ONLY valid JSON: {"improved_text": "..."}.
      ...
      if (result && result.improved_text) {
        setFormData(prev => ({ ...prev, raw_request: result.improved_text }));
      }
    } catch (err) {
      setError(`ไม่สามารถให้ AI เรียบเรียงได้: ${String(err)}`);
    } finally {
      setIsGeneratingAi(false);
    }
  };
  ...
  const prepareDraftReview = async () => {
    ...
    try {
      ...
      const quotation = buildQuotationDraft(target.projectName, quoteBrief);
